...
1. Update the CSV path inside `db/seeds/kdc_seed_load.sql`.
2. Ensure `local_infile` is enabled:

```sql
SHOW VARIABLES LIKE 'local_infile';
```

3. Run the seed script (requires `--local-infile=1`):
...
```bash
mysql --local-infile=1 --protocol=tcp -h 127.0.0.1 -P 3306 -u bsl -p bsl < db/seeds/kdc_seed_load.sql
```
