...
SRC_INDEX="${SRC_INDEX:-books_doc_v1_20260116_001}"
DST_INDEX="${DST_INDEX:-books_doc_v2_20260228_001}"
...
echo "[books-doc-reindex] OS_URL=$OS_URL"
echo "[books-doc-reindex] SRC_INDEX=$SRC_INDEX"
echo "[books-doc-reindex] DST_INDEX=$DST_INDEX"
echo "[books-doc-reindex] CUTOVER_ALIASES=$CUTOVER_ALIASES"
...
echo "[books-doc-reindex] running _reindex..."
curl -fsS -XPOST "$OS_URL/_reindex?wait_for_completion=true&refresh=true" \
  -H "Content-Type: application/json" \
  --data-binary "@$REINDEX_BODY_FILE" >/tmp/books_doc_reindex_result.json
cat /tmp/books_doc_reindex_result.json | jq '.total,.created,.updated,.failures'

echo "[books-doc-reindex] validating visibility field..."
MISSING_COUNT="$(curl -fsS -XPOST "$OS_URL/$DST_INDEX/_search" \
  -H "Content-Type: application/json" \
  -d '{"size":0,"query":{"bool":{"must_not":[{"exists":{"field":"is_hidden"}}]}}}' | jq -r '.hits.total.value')"
echo "[books-doc-reindex] missing is_hidden docs: $MISSING_COUNT"
if [ "$MISSING_COUNT" != "0" ]; then
  echo "Validation failed: some docs still missing is_hidden" >&2
  exit 1
fi

if [ "$CUTOVER_ALIASES" = "1" ]; then
  echo "[books-doc-reindex] cutting over aliases..."
  cat > /tmp/books_doc_alias_swap.json <<JSON
{
  "actions": [
...
JSON
  curl -fsS -XPOST "$OS_URL/_aliases" \
    -H "Content-Type: application/json" \
    --data-binary @/tmp/books_doc_alias_swap.json >/dev/null
  echo "[books-doc-reindex] aliases updated."
fi

echo "[books-doc-reindex] done."
