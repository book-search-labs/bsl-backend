...
def render_markdown(report: dict, baseline_name: str, candidate_name: str) -> str:
    lines = []
    lines.append(f"# Vector Eval Report ({baseline_name} -> {candidate_name})")
    lines.append("")
    ...
    for item in report.get("improved_cases", []):
        lines.append(
            f"- {item['qid']}: {item['query']} (ΔNDCG={item['delta']:+.4f})"
        )
...
def main() -> int:
    ...
    json_path = out_dir / f"vector_eval_{ts}.json"
    md_path = out_dir / f"vector_eval_{ts}.md"
