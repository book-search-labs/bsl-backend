...
def compare_with_baseline(
    ...
    *,
    max_execute_without_confirmation_total_increase: int,
    max_timeout_auto_cancel_ratio_drop: float,
    max_confirmation_latency_p95_sec_increase: float,
    ...
    base_execute_without_confirmation_total = _safe_int(base_summary.get("execute_without_confirmation_total"), 0)
    cur_execute_without_confirmation_total = _safe_int(current_summary.get("execute_without_confirmation_total"), 0)
    execute_without_confirmation_total_increase = max(
        0, cur_execute_without_confirmation_total - base_execute_without_confirmation_total
    )
    if execute_without_confirmation_total_increase > max(0, int(max_execute_without_confirmation_total_increase)):
        failures.append(
            "execute-without-confirmation regression: "
            f"baseline={base_execute_without_confirmation_total}, current={cur_execute_without_confirmation_total}, "
            f"allowed_increase={max(0, int(max_execute_without_confirmation_total_increase))}"
        )

    base_timeout_auto_cancel_ratio = _safe_float(base_summary.get("timeout_auto_cancel_ratio"), 1.0)
    cur_timeout_auto_cancel_ratio = _safe_float(current_summary.get("timeout_auto_cancel_ratio"), 1.0)
    timeout_auto_cancel_ratio_drop = max(0.0, base_timeout_auto_cancel_ratio - cur_timeout_auto_cancel_ratio)
    if timeout_auto_cancel_ratio_drop > max(0.0, float(max_timeout_auto_cancel_ratio_drop)):
        failures.append(
            "timeout auto-cancel ratio regression: "
            f"baseline={base_timeout_auto_cancel_ratio:.6f}, current={cur_timeout_auto_cancel_ratio:.6f}, "
            f"allowed_drop={float(max_timeout_auto_cancel_ratio_drop):.6f}"
        )

    base_confirmation_latency_p95_sec = _safe_float(base_summary.get("confirmation_latency_p95_sec"), 0.0)
    cur_confirmation_latency_p95_sec = _safe_float(current_summary.get("confirmation_latency_p95_sec"), 0.0)
    confirmation_latency_p95_sec_increase = max(
        0.0, cur_confirmation_latency_p95_sec - base_confirmation_latency_p95_sec
    )
    if confirmation_latency_p95_sec_increase > max(0.0, float(max_confirmation_latency_p95_sec_increase)):
        failures.append(
            "confirmation latency p95 regression: "
            f"baseline={base_confirmation_latency_p95_sec:.6f}, current={cur_confirmation_latency_p95_sec:.6f}, "
            f"allowed_increase={float(max_confirmation_latency_p95_sec_increase):.6f}"
        )
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--max-execute-without-confirmation-total-increase", type=int, default=0)
    parser.add_argument("--max-timeout-auto-cancel-ratio-drop", type=float, default=0.05)
    parser.add_argument("--max-confirmation-latency-p95-sec-increase", type=float, default=60.0)
...
def main() -> int:
    ...
    if args.baseline_report:
        ...
        baseline_failures = compare_with_baseline(
            baseline_payload,
            summary,
            max_execute_without_confirmation_total_increase=max(
                0, int(args.max_execute_without_confirmation_total_increase)
            ),
            max_timeout_auto_cancel_ratio_drop=max(0.0, float(args.max_timeout_auto_cancel_ratio_drop)),
            max_confirmation_latency_p95_sec_increase=max(0.0, float(args.max_confirmation_latency_p95_sec_increase)),
    ...
    payload = {
        ...
        "gate": {
            ...
            "thresholds": {
                ...
                "max_execute_without_confirmation_total_increase": int(
                    args.max_execute_without_confirmation_total_increase
                ),
                "max_timeout_auto_cancel_ratio_drop": float(args.max_timeout_auto_cancel_ratio_drop),
                "max_confirmation_latency_p95_sec_increase": float(args.max_confirmation_latency_p95_sec_increase),
