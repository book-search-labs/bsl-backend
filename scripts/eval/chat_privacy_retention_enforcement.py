...
def compare_with_baseline(
    ...
    *,
    max_expired_total_drop: int,
    max_purge_due_total_drop: int,
    max_purged_total_drop: int,
    max_purge_coverage_ratio_drop: float,
    max_purge_miss_total_increase: int,
    max_hold_violation_total_increase: int,
    max_invalid_retention_policy_total_increase: int,
    max_delete_audit_missing_total_increase: int,
    ...
    baseline_drop_pairs = [
        ("expired_total", max_expired_total_drop),
        ("purge_due_total", max_purge_due_total_drop),
        ("purged_total", max_purged_total_drop),
    ]
    ...
    base_purge_coverage_ratio = _safe_float(base_summary.get("purge_coverage_ratio"), 1.0)
    cur_purge_coverage_ratio = _safe_float(current_summary.get("purge_coverage_ratio"), 1.0)
    purge_coverage_ratio_drop = max(0.0, base_purge_coverage_ratio - cur_purge_coverage_ratio)
    if purge_coverage_ratio_drop > max(0.0, float(max_purge_coverage_ratio_drop)):
        failures.append(
            "purge_coverage_ratio regression: "
            f"baseline={base_purge_coverage_ratio:.6f}, current={cur_purge_coverage_ratio:.6f}, "
            f"allowed_drop={float(max_purge_coverage_ratio_drop):.6f}"
        )

    baseline_increase_pairs = [
        ("purge_miss_total", max_purge_miss_total_increase),
        ("hold_violation_total", max_hold_violation_total_increase),
        ("invalid_retention_policy_total", max_invalid_retention_policy_total_increase),
        ("delete_audit_missing_total", max_delete_audit_missing_total_increase),
    ]
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--max-expired-total-drop", type=int, default=10)
    parser.add_argument("--max-purge-due-total-drop", type=int, default=10)
    parser.add_argument("--max-purged-total-drop", type=int, default=10)
    parser.add_argument("--max-purge-coverage-ratio-drop", type=float, default=0.05)
    parser.add_argument("--max-purge-miss-total-increase", type=int, default=0)
    parser.add_argument("--max-hold-violation-total-increase", type=int, default=0)
    parser.add_argument("--max-invalid-retention-policy-total-increase", type=int, default=0)
    parser.add_argument("--max-delete-audit-missing-total-increase", type=int, default=0)
...
def main() -> int:
    ...
    if args.baseline_report:
        ...
        baseline_failures = compare_with_baseline(
            baseline_payload,
            summary,
            max_expired_total_drop=max(0, int(args.max_expired_total_drop)),
            max_purge_due_total_drop=max(0, int(args.max_purge_due_total_drop)),
            max_purged_total_drop=max(0, int(args.max_purged_total_drop)),
            max_purge_coverage_ratio_drop=max(0.0, float(args.max_purge_coverage_ratio_drop)),
            max_purge_miss_total_increase=max(0, int(args.max_purge_miss_total_increase)),
            max_hold_violation_total_increase=max(0, int(args.max_hold_violation_total_increase)),
            max_invalid_retention_policy_total_increase=max(0, int(args.max_invalid_retention_policy_total_increase)),
            max_delete_audit_missing_total_increase=max(0, int(args.max_delete_audit_missing_total_increase)),
    ...
    payload = {
        ...
        "gate": {
            ...
            "thresholds": {
                ...
                "max_expired_total_drop": int(args.max_expired_total_drop),
                "max_purge_due_total_drop": int(args.max_purge_due_total_drop),
                "max_purged_total_drop": int(args.max_purged_total_drop),
                "max_purge_coverage_ratio_drop": float(args.max_purge_coverage_ratio_drop),
                "max_purge_miss_total_increase": int(args.max_purge_miss_total_increase),
                "max_hold_violation_total_increase": int(args.max_hold_violation_total_increase),
                "max_invalid_retention_policy_total_increase": int(args.max_invalid_retention_policy_total_increase),
                "max_delete_audit_missing_total_increase": int(args.max_delete_audit_missing_total_increase),
