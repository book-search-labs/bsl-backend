...
def compare_with_baseline(
    ...
    *,
    max_detected_total_drop: int,
    max_protected_action_ratio_drop: float,
    max_unmasked_violation_total_increase: int,
    max_invalid_action_total_increase: int,
    max_unknown_pii_type_total_increase: int,
    max_false_positive_total_increase: int,
    max_missing_reason_total_increase: int,
    ...
    base_detected_total = _safe_int(base_summary.get("detected_total"), 0)
    cur_detected_total = _safe_int(current_summary.get("detected_total"), 0)
    detected_total_drop = max(0, base_detected_total - cur_detected_total)
    if detected_total_drop > max(0, int(max_detected_total_drop)):
        failures.append(
            "detected_total regression: "
            f"baseline={base_detected_total}, current={cur_detected_total}, "
            f"allowed_drop={max(0, int(max_detected_total_drop))}"
        )

    base_protected_action_ratio = _safe_float(base_summary.get("protected_action_ratio"), 1.0)
    cur_protected_action_ratio = _safe_float(current_summary.get("protected_action_ratio"), 1.0)
    protected_action_ratio_drop = max(0.0, base_protected_action_ratio - cur_protected_action_ratio)
    if protected_action_ratio_drop > max(0.0, float(max_protected_action_ratio_drop)):
        failures.append(
            "protected_action_ratio regression: "
            f"baseline={base_protected_action_ratio:.6f}, current={cur_protected_action_ratio:.6f}, "
            f"allowed_drop={float(max_protected_action_ratio_drop):.6f}"
        )

    baseline_increase_pairs = [
        ("unmasked_violation_total", max_unmasked_violation_total_increase),
        ("invalid_action_total", max_invalid_action_total_increase),
        ("unknown_pii_type_total", max_unknown_pii_type_total_increase),
        ("false_positive_total", max_false_positive_total_increase),
        ("missing_reason_total", max_missing_reason_total_increase),
    ]
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--max-detected-total-drop", type=int, default=10)
    parser.add_argument("--max-protected-action-ratio-drop", type=float, default=0.05)
    parser.add_argument("--max-unmasked-violation-total-increase", type=int, default=0)
    parser.add_argument("--max-invalid-action-total-increase", type=int, default=0)
    parser.add_argument("--max-unknown-pii-type-total-increase", type=int, default=0)
    parser.add_argument("--max-false-positive-total-increase", type=int, default=0)
    parser.add_argument("--max-missing-reason-total-increase", type=int, default=0)
...
def main() -> int:
    ...
    if args.baseline_report:
        ...
        baseline_failures = compare_with_baseline(
            baseline_payload,
            summary,
            max_detected_total_drop=max(0, int(args.max_detected_total_drop)),
            max_protected_action_ratio_drop=max(0.0, float(args.max_protected_action_ratio_drop)),
            max_unmasked_violation_total_increase=max(0, int(args.max_unmasked_violation_total_increase)),
            max_invalid_action_total_increase=max(0, int(args.max_invalid_action_total_increase)),
            max_unknown_pii_type_total_increase=max(0, int(args.max_unknown_pii_type_total_increase)),
            max_false_positive_total_increase=max(0, int(args.max_false_positive_total_increase)),
            max_missing_reason_total_increase=max(0, int(args.max_missing_reason_total_increase)),
    ...
    payload = {
        ...
        "gate": {
            ...
            "thresholds": {
                ...
                "max_detected_total_drop": int(args.max_detected_total_drop),
                "max_protected_action_ratio_drop": float(args.max_protected_action_ratio_drop),
                "max_unmasked_violation_total_increase": int(args.max_unmasked_violation_total_increase),
                "max_invalid_action_total_increase": int(args.max_invalid_action_total_increase),
                "max_unknown_pii_type_total_increase": int(args.max_unknown_pii_type_total_increase),
                "max_false_positive_total_increase": int(args.max_false_positive_total_increase),
                "max_missing_reason_total_increase": int(args.max_missing_reason_total_increase),
