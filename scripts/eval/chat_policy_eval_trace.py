...
def compare_with_baseline(
    ...
    *,
    ...
    max_missing_matched_rule_total_increase: int,
    max_unknown_final_action_total_increase: int,
    max_non_deterministic_key_total_increase: int,
    max_conflict_unresolved_total_increase: int,
    max_latency_p95_ms_increase: float,
    ...
    baseline_pairs = [
        ...
        ("missing_matched_rule_total", max_missing_matched_rule_total_increase),
        ("unknown_final_action_total", max_unknown_final_action_total_increase),
        ("non_deterministic_key_total", max_non_deterministic_key_total_increase),
        ("conflict_unresolved_total", max_conflict_unresolved_total_increase),
    ]
    ...
    base_latency_p95_ms = _safe_float(base_summary.get("latency_p95_ms"), 0.0)
    cur_latency_p95_ms = _safe_float(current_summary.get("latency_p95_ms"), 0.0)
    latency_p95_ms_increase = max(0.0, cur_latency_p95_ms - base_latency_p95_ms)
    if latency_p95_ms_increase > max(0.0, float(max_latency_p95_ms_increase)):
        failures.append(
            "latency p95 regression: "
            f"baseline={base_latency_p95_ms:.6f}, current={cur_latency_p95_ms:.6f}, "
            f"allowed_increase={float(max_latency_p95_ms_increase):.6f}"
        )
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--max-missing-matched-rule-total-increase", type=int, default=0)
    parser.add_argument("--max-unknown-final-action-total-increase", type=int, default=0)
    parser.add_argument("--max-non-deterministic-key-total-increase", type=int, default=0)
    parser.add_argument("--max-conflict-unresolved-total-increase", type=int, default=0)
    parser.add_argument("--max-latency-p95-ms-increase", type=float, default=300.0)
...
def main() -> int:
    ...
    if args.baseline_report:
        ...
        baseline_failures = compare_with_baseline(
            ...
            max_missing_matched_rule_total_increase=max(0, int(args.max_missing_matched_rule_total_increase)),
            max_unknown_final_action_total_increase=max(0, int(args.max_unknown_final_action_total_increase)),
            max_non_deterministic_key_total_increase=max(0, int(args.max_non_deterministic_key_total_increase)),
            max_conflict_unresolved_total_increase=max(0, int(args.max_conflict_unresolved_total_increase)),
            max_latency_p95_ms_increase=max(0.0, float(args.max_latency_p95_ms_increase)),
    ...
    payload = {
        ...
        "gate": {
            ...
            "thresholds": {
                ...
                "max_missing_matched_rule_total_increase": int(args.max_missing_matched_rule_total_increase),
                "max_unknown_final_action_total_increase": int(args.max_unknown_final_action_total_increase),
                "max_non_deterministic_key_total_increase": int(args.max_non_deterministic_key_total_increase),
                "max_conflict_unresolved_total_increase": int(args.max_conflict_unresolved_total_increase),
                "max_latency_p95_ms_increase": float(args.max_latency_p95_ms_increase),
