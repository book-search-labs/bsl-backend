...
def compare_with_baseline(
    ...
    *,
    max_sequence_valid_ratio_drop: float,
    max_validation_before_execute_ratio_drop: float,
    max_step_error_total_increase: int,
    max_reentry_success_ratio_drop: float,
    ...
    base_sequence_valid_ratio = _safe_float(base_summary.get("sequence_valid_ratio"), 1.0)
    cur_sequence_valid_ratio = _safe_float(current_summary.get("sequence_valid_ratio"), 1.0)
    sequence_valid_ratio_drop = max(0.0, base_sequence_valid_ratio - cur_sequence_valid_ratio)
    if sequence_valid_ratio_drop > max(0.0, float(max_sequence_valid_ratio_drop)):
        failures.append(
            "sequence valid ratio regression: "
            f"baseline={base_sequence_valid_ratio:.6f}, current={cur_sequence_valid_ratio:.6f}, "
            f"allowed_drop={float(max_sequence_valid_ratio_drop):.6f}"
        )

    base_validation_before_execute_ratio = _safe_float(base_summary.get("validation_before_execute_ratio"), 1.0)
    cur_validation_before_execute_ratio = _safe_float(current_summary.get("validation_before_execute_ratio"), 1.0)
    validation_before_execute_ratio_drop = max(0.0, base_validation_before_execute_ratio - cur_validation_before_execute_ratio)
    if validation_before_execute_ratio_drop > max(0.0, float(max_validation_before_execute_ratio_drop)):
        failures.append(
            "validation-before-execute ratio regression: "
            f"baseline={base_validation_before_execute_ratio:.6f}, current={cur_validation_before_execute_ratio:.6f}, "
            f"allowed_drop={float(max_validation_before_execute_ratio_drop):.6f}"
        )

    base_step_error_total = _safe_int(base_summary.get("step_error_total"), 0)
    cur_step_error_total = _safe_int(current_summary.get("step_error_total"), 0)
    step_error_total_increase = max(0, cur_step_error_total - base_step_error_total)
    if step_error_total_increase > max(0, int(max_step_error_total_increase)):
        failures.append(
            "step error total regression: "
            f"baseline={base_step_error_total}, current={cur_step_error_total}, "
            f"allowed_increase={max(0, int(max_step_error_total_increase))}"
        )

    base_reentry_success_ratio = _safe_float(base_summary.get("reentry_success_ratio"), 1.0)
    cur_reentry_success_ratio = _safe_float(current_summary.get("reentry_success_ratio"), 1.0)
    reentry_success_ratio_drop = max(0.0, base_reentry_success_ratio - cur_reentry_success_ratio)
    if reentry_success_ratio_drop > max(0.0, float(max_reentry_success_ratio_drop)):
        failures.append(
            "reentry success ratio regression: "
            f"baseline={base_reentry_success_ratio:.6f}, current={cur_reentry_success_ratio:.6f}, "
            f"allowed_drop={float(max_reentry_success_ratio_drop):.6f}"
        )
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--max-sequence-valid-ratio-drop", type=float, default=0.05)
    parser.add_argument("--max-validation-before-execute-ratio-drop", type=float, default=0.05)
    parser.add_argument("--max-step-error-total-increase", type=int, default=0)
    parser.add_argument("--max-reentry-success-ratio-drop", type=float, default=0.10)
...
def main() -> int:
    ...
    if args.baseline_report:
        ...
        baseline_failures = compare_with_baseline(
            baseline_payload,
            summary,
            max_sequence_valid_ratio_drop=max(0.0, float(args.max_sequence_valid_ratio_drop)),
            max_validation_before_execute_ratio_drop=max(0.0, float(args.max_validation_before_execute_ratio_drop)),
            max_step_error_total_increase=max(0, int(args.max_step_error_total_increase)),
            max_reentry_success_ratio_drop=max(0.0, float(args.max_reentry_success_ratio_drop)),
    ...
    payload = {
        ...
        "gate": {
            ...
            "thresholds": {
                ...
                "max_sequence_valid_ratio_drop": float(args.max_sequence_valid_ratio_drop),
                "max_validation_before_execute_ratio_drop": float(args.max_validation_before_execute_ratio_drop),
                "max_step_error_total_increase": int(args.max_step_error_total_increase),
                "max_reentry_success_ratio_drop": float(args.max_reentry_success_ratio_drop),
