...
def compare_with_baseline(
    ...
    *,
    max_mock_total_drop: int,
    max_real_total_drop: int,
    max_parity_mismatch_total_increase: int,
    max_non_deterministic_total_increase: int,
    max_missing_mode_total_increase: int,
    max_invalid_result_total_increase: int,
    max_missing_seed_total_increase: int,
    max_missing_response_hash_total_increase: int,
    max_parity_match_ratio_drop: float,
    ...
    baseline_drop_pairs = [
        ("mock_total", max_mock_total_drop),
        ("real_total", max_real_total_drop),
    ]
    ...
    baseline_increase_pairs = [
        ("parity_mismatch_total", max_parity_mismatch_total_increase),
        ("non_deterministic_total", max_non_deterministic_total_increase),
        ("missing_mode_total", max_missing_mode_total_increase),
        ("invalid_result_total", max_invalid_result_total_increase),
        ("missing_seed_total", max_missing_seed_total_increase),
        ("missing_response_hash_total", max_missing_response_hash_total_increase),
    ]
    ...
    base_parity_match_ratio = _safe_float(base_summary.get("parity_match_ratio"), 1.0)
    cur_parity_match_ratio = _safe_float(current_summary.get("parity_match_ratio"), 1.0)
    parity_match_ratio_drop = max(0.0, base_parity_match_ratio - cur_parity_match_ratio)
    if parity_match_ratio_drop > max(0.0, float(max_parity_match_ratio_drop)):
        failures.append(
            "parity_match_ratio regression: "
            f"baseline={base_parity_match_ratio:.6f}, current={cur_parity_match_ratio:.6f}, "
            f"allowed_drop={float(max_parity_match_ratio_drop):.6f}"
        )
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--max-mock-total-drop", type=int, default=10)
    parser.add_argument("--max-real-total-drop", type=int, default=10)
    parser.add_argument("--max-parity-mismatch-total-increase", type=int, default=0)
    parser.add_argument("--max-non-deterministic-total-increase", type=int, default=0)
    parser.add_argument("--max-missing-mode-total-increase", type=int, default=0)
    parser.add_argument("--max-invalid-result-total-increase", type=int, default=0)
    parser.add_argument("--max-missing-seed-total-increase", type=int, default=0)
    parser.add_argument("--max-missing-response-hash-total-increase", type=int, default=0)
    parser.add_argument("--max-parity-match-ratio-drop", type=float, default=0.05)
...
def main() -> int:
    ...
    if args.baseline_report:
        ...
        baseline_failures = compare_with_baseline(
            baseline_payload,
            summary,
            max_mock_total_drop=max(0, int(args.max_mock_total_drop)),
            max_real_total_drop=max(0, int(args.max_real_total_drop)),
            max_parity_mismatch_total_increase=max(0, int(args.max_parity_mismatch_total_increase)),
            max_non_deterministic_total_increase=max(0, int(args.max_non_deterministic_total_increase)),
            max_missing_mode_total_increase=max(0, int(args.max_missing_mode_total_increase)),
            max_invalid_result_total_increase=max(0, int(args.max_invalid_result_total_increase)),
            max_missing_seed_total_increase=max(0, int(args.max_missing_seed_total_increase)),
            max_missing_response_hash_total_increase=max(0, int(args.max_missing_response_hash_total_increase)),
            max_parity_match_ratio_drop=max(0.0, float(args.max_parity_match_ratio_drop)),
    ...
    payload = {
        ...
        "gate": {
            ...
            "thresholds": {
                ...
                "max_mock_total_drop": int(args.max_mock_total_drop),
                "max_real_total_drop": int(args.max_real_total_drop),
                "max_parity_mismatch_total_increase": int(args.max_parity_mismatch_total_increase),
                "max_non_deterministic_total_increase": int(args.max_non_deterministic_total_increase),
                "max_missing_mode_total_increase": int(args.max_missing_mode_total_increase),
                "max_invalid_result_total_increase": int(args.max_invalid_result_total_increase),
                "max_missing_seed_total_increase": int(args.max_missing_seed_total_increase),
                "max_missing_response_hash_total_increase": int(args.max_missing_response_hash_total_increase),
                "max_parity_match_ratio_drop": float(args.max_parity_match_ratio_drop),
