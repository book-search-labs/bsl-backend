...
echo "[1/6] Contract validation (optional)"
...
echo "[2/6] Contract compatibility gate (optional)"
if [ -n "$PYTHON_BIN" ]; then
...
echo "[3/6] Feature spec validation (optional)"
if [ -n "$PYTHON_BIN" ]; then
...
echo "[4/6] Offline eval gate (optional)"
...
echo "[5/6] Canonical quality checks (optional)"
