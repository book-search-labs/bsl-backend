...
FEEDBACK_REGRESSION_CANDIDATE_JSON="${CHAT_FEEDBACK_REGRESSION_CANDIDATE_JSON:-$ROOT_DIR/evaluation/chat/feedback_regression_fixture_candidates.json}"
FEEDBACK_REGRESSION_CANDIDATE_MD="${CHAT_FEEDBACK_REGRESSION_CANDIDATE_MD:-$ROOT_DIR/tasks/backlog/generated/chat_feedback_regression_fixture_candidates.md}"
FEEDBACK_REGRESSION_CANDIDATE_ALLOW_EMPTY="${CHAT_FEEDBACK_REGRESSION_CANDIDATE_ALLOW_EMPTY:-1}"
FEEDBACK_REGRESSION_APPLY_REPORT_JSON="${CHAT_FEEDBACK_REGRESSION_APPLY_REPORT_JSON:-$ROOT_DIR/evaluation/chat/feedback_regression_fixture_apply_report.json}"
FEEDBACK_REGRESSION_APPLY_REPORT_MD="${CHAT_FEEDBACK_REGRESSION_APPLY_REPORT_MD:-$ROOT_DIR/tasks/backlog/generated/chat_feedback_regression_fixture_apply_report.md}"
FEEDBACK_REGRESSION_APPLY_ALLOW_REVIEW="${CHAT_FEEDBACK_REGRESSION_APPLY_ALLOW_REVIEW:-0}"
FEEDBACK_REGRESSION_APPLY_MAX_ADD="${CHAT_FEEDBACK_REGRESSION_APPLY_MAX_ADD:-0}"
FEEDBACK_REGRESSION_APPLY_DRY_RUN="${CHAT_FEEDBACK_REGRESSION_APPLY_DRY_RUN:-1}"
...
REGRESSION_INGEST_DIR="${CHAT_REGRESSION_INGEST_DIR:-$ROOT_DIR/tasks/backlog/generated}"
...
FEEDBACK_REGRESSION_FIXTURE_ARGS=(
  "$ROOT_DIR/scripts/chat/build_regression_seed_fixture.py"
  --seeds-json "$FEEDBACK_REGRESSION_SEEDS_JSON"
  --base-fixture "$REGRESSION_FIXTURE"
  --output-json "$FEEDBACK_REGRESSION_CANDIDATE_JSON"
  --output-md "$FEEDBACK_REGRESSION_CANDIDATE_MD"
)
if [ "$FEEDBACK_REGRESSION_CANDIDATE_ALLOW_EMPTY" = "1" ]; then
  FEEDBACK_REGRESSION_FIXTURE_ARGS+=(--allow-empty)
fi
"$PYTHON_BIN" "${FEEDBACK_REGRESSION_FIXTURE_ARGS[@]}"

FEEDBACK_REGRESSION_APPLY_ARGS=(
  "$ROOT_DIR/scripts/chat/apply_regression_fixture_candidates.py"
  --fixture "$REGRESSION_FIXTURE"
  --candidates-json "$FEEDBACK_REGRESSION_CANDIDATE_JSON"
  --report-json "$FEEDBACK_REGRESSION_APPLY_REPORT_JSON"
  --report-md "$FEEDBACK_REGRESSION_APPLY_REPORT_MD"
  --max-add "$FEEDBACK_REGRESSION_APPLY_MAX_ADD"
)
if [ "$FEEDBACK_REGRESSION_CANDIDATE_ALLOW_EMPTY" = "1" ]; then
  FEEDBACK_REGRESSION_APPLY_ARGS+=(--allow-empty)
fi
if [ "$FEEDBACK_REGRESSION_APPLY_ALLOW_REVIEW" = "1" ]; then
  FEEDBACK_REGRESSION_APPLY_ARGS+=(--allow-review-required)
fi
if [ "$FEEDBACK_REGRESSION_APPLY_DRY_RUN" = "1" ]; then
  FEEDBACK_REGRESSION_APPLY_ARGS+=(--dry-run)
fi
"$PYTHON_BIN" "${FEEDBACK_REGRESSION_APPLY_ARGS[@]}"
...
if [ "$AGENT_SUMMARY_ENABLED" = "1" ]; then
  AGENT_SUMMARY_ARGS=(
    ...
    --out "$RECOMMEND_REPORT_OUT"
    --max-age-minutes "$AGENT_SUMMARY_MAX_AGE_MINUTES"
  )
