...
public class MockPaymentGateway implements PaymentGateway {
    ...
    public CheckoutSession createCheckoutSession(CreateCheckoutSessionRequest request) {
        String sessionId = "mock-" + request.paymentId() + "-" + UUID.randomUUID().toString().substring(0, 8);
    ...
    public MockCompletionDecision completeMock(long paymentId, String result) {
        ...
        if (success) {
            return MockCompletionDecision.captured("CAPTURED", "mock-" + paymentId, "CAPTURE_SUCCEEDED");
        }
        return MockCompletionDecision.failed("FAILED", "mock_failed", "CAPTURE_FAILED");
    }
