...
public class RefundServiceClient {
    ...
    public RefundServiceClient(
        @Qualifier("refundServiceRestTemplate") RestTemplate restTemplate,
        DownstreamProperties downstreamProperties,
        ObjectMapper objectMapper
    ) {
        this.restTemplate = restTemplate;
        this.properties = downstreamProperties.getRefundService();
    ...
    public ResponseEntity<String> exchange(
        ...
        try {
            ...
            throw new DownstreamException(HttpStatus.SERVICE_UNAVAILABLE, "refund_service_timeout", "Refund service timeout");
        } catch (HttpStatusCodeException ex) {
            HttpStatus status = HttpStatus.resolve(ex.getStatusCode().value());
            if (status == null) {
                status = HttpStatus.SERVICE_UNAVAILABLE;
            }
            String defaultCode = status.is4xxClientError() ? "refund_service_bad_request" : "refund_service_error";
