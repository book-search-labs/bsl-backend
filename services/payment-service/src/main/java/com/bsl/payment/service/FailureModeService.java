...
public class FailureModeService {
    ...
    public Map<String, Object> setMode(String value) {
        ...
        mode.set(next);
        return Map.of("service", "payment-service", "failure_mode", next.name());
    }
    ...
    public FailureMode beforeSideEffect() {
        ...
        if (selected == FailureMode.FAIL_500) {
            throw new ApiException(HttpStatus.INTERNAL_SERVER_ERROR, "failure_mode_500", "payment failure mode FAIL_500");
        }
        if (selected == FailureMode.TIMEOUT) {
            sleep();
            throw new ApiException(HttpStatus.GATEWAY_TIMEOUT, "failure_mode_timeout", "payment failure mode TIMEOUT");
        }
        return selected;
    }
